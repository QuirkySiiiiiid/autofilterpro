...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="❁ ԃσɯɳʅσαԃ Ⴆყ : @WilsonVerse")
        else:
            dump_file=await message.reply_video(url,caption="❁ ԃσɯɳʅσαԃ Ⴆყ : @WilsonVerse")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="❁ ԃσɯɳʅσαԃ Ⴆყ : @WilsonVerse")
                   else:
                       dump_file=await message.reply_video(content_value, caption="❁ ԃσɯɳʅσαԃ Ⴆყ : @WilsonVerse")
               except:
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="❁ ԃσɯɳʅσαԃ Ⴆყ : @WilsonVerse")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="❁ ԃσɯɳʅσαԃ Ⴆყ : @WilsonVerse")
                  except:
          ...
          #     await message.reply(tracemsg)
            ##optinal 
            await message.reply(f"400: Sorry, Unable To Find It  try another or report it  to @Anime106_Request_bot")
            ...
            if 'downfile' in locals():
                os.remove(downfile)
            await message.reply("<a href='https://t.me/WilsonVerse'>Q̳̿͟͞u̳̿͟͞i̳̿͟͞r̳̿͟͞k̳̿͟͞y̳̿͟͞</a>")
